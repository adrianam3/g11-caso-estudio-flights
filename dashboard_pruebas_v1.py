# dashboard_pruebas.py
"""
Dashboard unificado - Predicción + Análisis + Métricas
- Mantiene diseño: sidebar, 3 pestañas, 3 columnas en Predicción
- Usa flights_clean.csv en la ruta indicada por el usuario
- Muestra aeropuertos y aerolíneas como "COD - Nombre"
- Hora de salida: select hour + minute -> convertido a HHMM
- No pide hora de llegada: usa mediana histórica por ruta
- Muestra distancia (DISTANCE) desde CSV
- Muestra meses y días como texto
- Botón "Predecir Retraso" reducido mediante columna angosta (sin CSS/HTML)
"""
...
# -------------------------
# 2) Cargar dataset y tabla de rutas
# -------------------------
@st.cache_data
...
def main(artifacts):
    ...
    csv_input = st.sidebar.text_input("Ruta al CSV (si quieres cambiarla)", value=str(DATA_PATH))
    st.sidebar.markdown("---")
    ...
    # Recargar dataset si el usuario cambió ruta
    ...
    # Pestañas
    tab1, tab2, tab3 = st.tabs(["✈️ Predicción Interactiva", "📊 Análisis de Datos Históricos", "📈 Métricas del Modelo"])
    ...
    with tab2:
        ...
        if flights_clean is None or flights_clean.empty:
            ...
            with colf1:
                filtro_airline = st.selectbox("Filtrar por Aerolínea (Todas)", options=["Todas"] + sorted(flights_clean["AIRLINE"].dropna().unique().tolist()))
            with colf2:
                only_retrasos = st.checkbox("Mostrar solo vuelos retrasados (ARRIVAL_DELAY > 0)", value=True)
    ...
    with tab3:
        ...
        st.warning("Importante: Estos son resultados de ejemplo. Sustituye por tus métricas reales si las tienes.")
        st.subheader("KPIs de Validación (ejemplo)")
        k1,k2,k3,k4 = st.columns(4)
        k1.metric("ROC-AUC","0.6212")
        k2.metric("PR-AUC","0.2421")
        k3.metric("Mejor F1","0.3316")
        k4.metric("Umbral F1 óptimo","0.708")

        st.divider()

        # imágenes de evaluación si existen
        img_folder = PROJECT_ROOT / "images"
        img_cm = img_folder / "planning_confusion_matrix.png"
        img_fi = img_folder / "planning_feature_importance.png"
        pc1, pc2 = st.columns(2)
        with pc1:
            if img_cm.exists():
                st.image(str(img_cm), caption="Matriz de Confusión (Modelo Planificación)")
            else:
                st.warning("No se encontró 'planning_confusion_matrix.png' en /images/")
        with pc2:
            if img_fi.exists():
                st.image(str(img_fi), caption="Importancia de Features (Modelo Planificación)")
            else:
                st.warning("No se encontró 'planning_feature_importance.png' en /images/")
